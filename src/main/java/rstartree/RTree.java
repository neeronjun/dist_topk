...
import static rstartree.geometry.Geometries.rectangle;
...
import rstartree.geometry.Intersects;
...
import rstartree.internal.operators.OperatorBoundedPriorityQueue;
...
public final class RTree<T, S extends Geometry> {
    ...
    private Rectangle calculateMaxView(RTree<T, S> tree) {
        return tree.entries().reduce(Optional.<Rectangle> absent(),
                ...
                }).toBlocking().single().or(Geometries.rectangle(0,0,0, 0, 0, 0));
    }
    ...
    private String asString(Node<T, S> node, String margin) {
        ...
        s1 = node.toString();
        //int index = s1.length();
        int startIndex = s1.indexOf("l");
        //     s1.replace("com.github.davidmoten.rtree.internal", "");
        s.append(s1.substring(startIndex+2));
        s.append("\t");
    ...
    private void updateProb(Node<T,S> node){
        if (node instanceof NonLeaf) {
            NonLeaf<T, S> n = (NonLeaf<T, S>) node;
            for (int i = 0; i < n.count(); i++) {
                Node<T, S> child = n.child(i);
                ((NonLeaf) node).child(i);
            }
        } else {
            Leaf<T, S> leaf = (Leaf<T, S>) node;
