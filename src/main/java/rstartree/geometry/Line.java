...
public final class Line implements Geometry {
    ...
    private Line(int id, float x1, float y1, float x2, float y2, float prob) {
        this.id = id;
        this.x1 = x1;
        this.y1 = y1;
        this.x2 = x2;
    ...
    static Line create(int id, float x1, float y1, float x2, float y2, float prob) {
        return new Line(id, x1, y1, x2, y2, prob);
    }

    static Line create(int id, double x1, double y1, double x2, double y2, double prob) {
        return new Line(id, (float) x1, (float) y1, (float) x2, (float) y2, (float) prob);
    }
    ...
    public Rectangle mbr() {
        return Geometries.rectangle(id, Math.min(x1, x2), Math.min(y1, y2), Math.max(x1, x2),
                Math.max(y1, y2), prob);
    }
